...
const OrderType = () => {
  ...
  const { data, isFetching, isError, isSuccess } = useKeywordQuery(params);
  const { enqueueSnackbar } = useSnackbar();

  const [archiveKeyword, { isLoading: loadingArchive }] =
    useArchiveKeywordMutation();
  const [importKeyword, { isLoading: loadingImport }] =
    useImportKeywordMutation();
  ...
  const handleArchive = async () => {
    try {
      await archiveKeyword(type).unwrap();
      enqueueSnackbar(
        `Order type has been ${params?.status === "active" ? "archived" : "restored"}`,
        { variant: "success" }
      );
      dispatch(resetModal());
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importKeyword(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Box sx={{ marginLeft: 2 }}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} type?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this type?`}
