...
import HriModal from "../../../components/modal/HriModal";
...
const Hri = () => {
  ...
  const { data, isFetching, isError, isSuccess } = useHriQuery(params);
  const { enqueueSnackbar } = useSnackbar();

  const [archiveHri, { isLoading: loadingArchive }] = useArchiveHriMutation();
  const [importHri, { isLoading: loadingImport }] = useImportHriMutation();
  ...
  const handleArchive = async () => {
    try {
      await archiveHri(hri).unwrap();
      enqueueSnackbar(
        `Hri has been ${params?.status === "active" ? "archived" : "restored"}`,
        { variant: "success" }
      );
      dispatch(resetModal());
  ...
  const handleImport = async () => {
    ...
    try {
      const res = await importHri(mapped).unwrap();
      dispatch(resetModal());
  ...
  return (
    <Box sx={{ marginLeft: 2 }}>
      ...
      <AppPrompt
        open={archive}
        image={warning}
        title={`${params?.status === "active" ? "Archive" : "Restore"} hri?`}
        message={`Are you sure you want to ${params?.status === "active" ? "archive" : "restore"} this hri?`}
